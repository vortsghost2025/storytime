...
# Falls back to manual escaping if jq is unavailable.
...
json_obj() {
	if $HAS_JQ; then
		...
		# Build the jq expression from the key names
		local keys=()
		for ((i = 0; i < ${#args[@]}; i += 3)); do
			keys+=("${args[i+1]}")
		done
		# Simpler approach: rebuild args and expression together
		local jq_args=()
		local pairs=()
		local orig_args=("$@")
		local i=0
		# Re-parse since we shifted
		set -- "${orig_args[@]}" 2>/dev/null || true
		while [[ $# -ge 2 ]]; do
			jq_args+=("--arg" "$1" "$2")
			pairs+=("\"$(printf '%s' "$1")\": \$$1")
			shift 2
...
# Build a JSON object with maxTokens as a numeric field.
# All other fields are strings.
# Usage: json_send_obj key1 value1 ...
json_send_obj() {
	if $HAS_JQ; then
		local jq_args=()
		local pairs=()
		...
		while [[ $# -ge 2 ]]; do
			...
			if [[ "$key" == "maxTokens" ]]; then
				...
				local escaped
				escaped=$(printf '%s' "$val" | sed 's/\\/\\\\/g; s/"/\\"/g; s/\t/\\t/g; s/\n/\\n/g; s/\r/\\r/g')
				result+="\"$key\":\"$escaped\""
			fi
			shift 2
		done
...
cmd_send() {
	...
	# Build JSON payload in one pass using json_send_obj
	local json_args=(from kilo to "$agent" message "$message" type "$msg_type")
	if [[ -n "$command" ]]; then
		json_args+=(command "$command")
	fi
	if [[ -n "$model" ]]; then
		json_args+=(model "$model")
	fi
	if [[ -n "$max_tokens" ]]; then
		json_args+=(maxTokens "$max_tokens")
	fi
	local json
	json=$(json_send_obj "${json_args[@]}")
...
cmd_broadcast() {
	...
	local json
	json=$(json_obj from kilo to all message "$message" type chat)
...
cmd_workflow() {
	...
	case "$workflow_name" in
		review-build)
			...
			local send_json
			send_json=$(json_obj from kilo to builder message "$build_command" type task_assignment command "$build_command")
			...
			local review_msg="Review build output: ${review_input}"
			local review_json
			review_json=$(json_obj from kilo to reviewer message "$review_msg" type task_assignment)
